...
def item_based_knn(uid="1"):
    ...
    # function for predicting 20 recommended items for a specific user
    def predict_user(model, user_id, top_k=20):
    ...
    # 1.1) Train once and save for jaccard
    train_and_save_model("itemknn_jaccard")

    # 1.2) Later: load and predict for jaccard
    jaccard_model = load_model("itemknn_jaccard")
    jaccard_recs_for_u100 = predict_user(jaccard_model, uid, top_k=20)
    print("recommendation 20 for jaccard: ") 
    print(' '.join(jaccard_recs_for_u100))

    # 2.1) Train once and save for cosine
    train_and_save_model("itemknn_cosine")

    # 2.2) Later: load and predict for cosine
    cosine_model = load_model("itemknn_cosine")
    cosine_recs_for_u100 = predict_user(cosine_model, uid, top_k=20)
    print("recommendation 20 for cosine: ")
    print(' '.join(cosine_recs_for_u100))

if __name__ == "__main__":
    # Please put in your user id.
