...
def ndcg_evaluation():
    ...
    results_list = []
    for i in [5, 10]:
        results_list.append(run_nfold_cv(
    ...
    print("NDCG@20, cosine w/ 5fold = ", [float(f"{v:.4f}") for v in results_list[0]["cosine"]["mean"]])
    print("NDCG@20, jaccard w/ 5fold = ", [float(f"{v:.4f}") for v in results_list[0]["jaccard"]["mean"]])
    print("NDCG@20, pearson w/ 5fold = ", [float(f"{v:.4f}") for v in results_list[0]["pearson"]["mean"]])
    print("NDCG@20, cosine w/ 10fold = ", [float(f"{v:.4f}") for v in results_list[1]["cosine"]["mean"]])
    print("NDCG@20, jaccard w/ 10fold = ", [float(f"{v:.4f}") for v in results_list[1]["jaccard"]["mean"]])
    print("NDCG@20, pearson w/ 10fold = ", [float(f"{v:.4f}") for v in results_list[1]["pearson"]["mean"]])
    ...
    print("NDCG@20, cosine w/o nfold = ", ndcg_cosine)
    print("NDCG@20, jaccard w/o nfold = ", ndcg_jaccard)
    print("NDCG@20, pearson w/o nfold = ", ndcg_pearson)

    # ---------- plot ----------
    fig, ax = plt.subplots(figsize=(8,5))
    ax.plot(K_VALUES,  results_list[0]["cosine"]["mean"],  marker="o", color="green", label="Cosine w/ 5fold")
    ax.plot(K_VALUES, results_list[0]["jaccard"]["mean"], marker="s", color="red", label="Jaccard w/ 5fold")
    ax.plot(K_VALUES, results_list[0]["pearson"]["mean"], marker="+", color="orange", label="Pearson w/ 5fold")
    ax.plot(K_VALUES,  results_list[1]["cosine"]["mean"],  marker="o", color="brown", label="Cosine w/ 10fold")
    ax.plot(K_VALUES, results_list[1]["jaccard"]["mean"], marker="s", color="grey", label="Jaccard w/ 10fold")
    ax.plot(K_VALUES, results_list[1]["pearson"]["mean"], marker="+", color="purple", label="Pearson w/ 10fold")
    ax.plot(K_VALUES, ndcg_cosine, marker="o", color="blue", label="Cosine w/o nfold")
    ax.plot(K_VALUES, ndcg_jaccard, marker="s", color="pink", label="Jaccard w/o nfold")
    ax.plot(K_VALUES, ndcg_pearson, marker="+", color="yellow", label="Pearson w/o nfold")
    ax.set_xlabel("Neighbor list size (k)")
    ax.set_ylabel("NDCG@20")
    ax.set_title("NDCG@20 vs. k-size")
    ax.grid(True, linestyle="--", alpha=0.6)
    ax.legend()
    plt.tight_layout()
    plt.savefig("item-based_knn_entire_program_v2.jpg")
