def cosine_similarity():
    ...
    with open("recommendations_cosine10_loo.txt", "w", newline="", encoding="utf-8") as g:
...
def jaccard_similarity():
    ...
    out_path = Path("recommendations_jaccard10_loo.txt")
...
def pearson_similarity():
    ...
    with open("recommendations_pearson10_loo.txt", "w", encoding="utf-8") as g:
...
def ndcg_evaluation():
    ...
    def run_leave_one_out(
        ...
        """
        Leave-one-out evaluation. For each repeat:
        - pick one random item per user as test (if user has >=2 items)
        - build neighbors on the remaining interactions
        - recommend for *all* users using their train subset
        - compute NDCG@20 against the held-out item
        Aggregates NDCG over repeats (mean across repeats).
        """
        assert build_neighbors_fn and recommend_all_fn and average_ndcg_fn, \
            "Provide build_neighbors_fn, recommend_all_fn, and average_ndcg_fn."

        sim2scores = {s: np.zeros((repeats, len(k_values)), dtype=np.float64)
                    for s in sim_types}
        ...
        for r in range(repeats):
            ...
            # 2) for each k & similarity, build neighbors and evaluate
            for k_idx, k in enumerate(k_values):
                for sim in sim_types:
                    neighbors, popular = build_neighbors_fn(train_ui, k, sim_type=sim)

                    # recommend only for users that have a train subset (i.e., keys in train_ui)
                    recs = recommend_all_fn(train_ui, neighbors, popular, topn=topn)

                    # evaluate against the held-out items
                    ndcg = average_ndcg_fn(recs, test_ui, k=topn)
                    sim2scores[sim][r, k_idx] = ndcg

            print(f"[LOO] Completed repeat {r+1}/{repeats}")

        # 3) aggregate across repeats
        results = {}
        for sim in sim_types:
            mat = sim2scores[sim]          # shape: repeats x len(k_values)
    ...
    loo_results = run_leave_one_out(
        ...
        topn=20,
        seed=42,
        repeats=3,  # try >1 for stabler averages
        build_neighbors_fn=build_neighbors,
        recommend_all_fn=recommend_all_gpu_wrapper,
        average_ndcg_fn=average_ndcg
    )
        
    print("NDCG@20, cosine = ",  [f"{v:.4f}" for v in loo_results["cosine"]["mean"]])
    print("NDCG@20, jaccard = ",  [f"{v:.4f}" for v in loo_results["jaccard"]["mean"]])
    print("NDCG@20, pearson = ",  [f"{v:.4f}" for v in loo_results["pearson"]["mean"]])

    # ---------- plot ----------
    import matplotlib.pyplot as plt

    def plot_loo(results, title="Leave-one-out: NDCG@20 vs. k"):
        plt.figure(figsize=(8,5))
        for sim, payload in results.items():
            ks  = payload["k_values"]
            mu  = payload["mean"]
            std = payload["std"]
            plt.plot(ks, mu, marker="o", label=f"{sim} (mean)")
            # (optional) add ±1 std band
            # plt.fill_between(ks, np.array(mu)-std, np.array(mu)+std, alpha=0.15)
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig("item_based_knn_loo.jpg")
